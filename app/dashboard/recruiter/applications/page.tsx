...
import { ClipboardList, ArrowRight, FileText, TrendingUp, Star } from "lucide-react";
import { StatusButton } from "./status-button";

const pipelineOrder = ["applied", "reviewing", "interview", "offer", "rejected"];

const statusConfig: Record<string, { dot: string; label: string; border: string; bg: string; text: string }> = {
  applied:   { dot: "bg-zinc-400",    label: "Applied",   border: "border-zinc-700/50",     bg: "bg-zinc-900/60",       text: "text-zinc-300"    },
  pending:   { dot: "bg-zinc-400",    label: "Applied",   border: "border-zinc-700/50",     bg: "bg-zinc-900/60",       text: "text-zinc-300"    },
  reviewing: { dot: "bg-amber-400",   label: "Reviewing", border: "border-amber-500/30",    bg: "bg-amber-500/[0.06]",  text: "text-amber-300"   },
  interview: { dot: "bg-sky-400",     label: "Interview", border: "border-sky-500/30",      bg: "bg-sky-500/[0.06]",    text: "text-sky-300"     },
  offer:     { dot: "bg-emerald-400", label: "Offer",     border: "border-emerald-500/30",  bg: "bg-emerald-500/[0.06]",text: "text-emerald-300" },
  rejected:  { dot: "bg-rose-400",    label: "Rejected",  border: "border-rose-500/30",     bg: "bg-rose-500/[0.06]",   text: "text-rose-300"    },
};
...
export default async function ApplicationsPage() {
  ...
  const grouped = pipelineOrder.reduce(
    (acc, status) => {
      acc[status] = applications.filter(
        (a) => (a.status ?? "applied") === status || (status === "applied" && a.status === "pending"),
      );
      return acc;
    },
    ...
    <div className="mx-auto max-w-5xl space-y-8 px-4 py-8">
      <div className="pb-2">
        ...
        <p className="mt-1 text-sm text-zinc-500">
          {applications.length} total · {activeCount} active in pipeline
        </p>
      </div>
      ...
      {applications.length === 0 ? (
        <div className="rounded-2xl border border-dashed border-zinc-800/60 py-16 text-center backdrop-blur-sm">
          ...
          <p className="mt-1 text-sm text-zinc-600">Applications will appear here when candidates apply</p>
        </div>
      ) : (
                        ...
                        <div className="min-w-0 flex-1">
                          <div className="mb-1 flex flex-wrap items-center gap-2">
                            ...
                            <StatusButton
                              ...
                              initialStatus={application.status ?? "applied"}
                            />
                          </div>

                          <p className="mb-3 text-sm text-zinc-400">
                            Applied for:{" "}
                            <span className="font-medium text-zinc-200">{application.jobTitle}</span>
                          </p>
                          ...
                          <div className="mb-2 flex flex-wrap gap-2">
                            ...
                            {application.resumeScore && (
                              <Pill icon={FileText}>Resume {application.resumeScore}/100</Pill>
                            )}
                          </div>
