...
function MatchBadge({ score }: { score: number }) {
  ...
  const barColor =
    score >= 70 ? "bg-emerald-400" : score >= 40 ? "bg-amber-400" : "bg-zinc-600";
  return (
    <span className={`inline-flex items-center gap-1.5 rounded-full border px-2.5 py-0.5 font-mono text-[10px] uppercase tracking-wider ${color}`}>
      <Triangle className="h-2.5 w-2.5 fill-current" />
      <span className="tabular-nums">{score}</span>
      <span className="text-[9px] opacity-60">/ 100</span>
      <span className="h-1 w-8 overflow-hidden rounded-full opacity-20" style={{ background: "currentColor" }}>
        <span className={`block h-full rounded-full ${barColor}`} style={{ width: `${score}%`, opacity: 1 }} />
      </span>
    </span>
  );
}
...
function JobCard({ job, isWatchlisted }: { job: Job; isWatchlisted: boolean }) {
  ...
  return (
    <div
      className="group flex flex-col gap-4 rounded-lg border border-zinc-800/60 bg-zinc-950/50 p-5 backdrop-blur-sm transition-all duration-200 hover:border-zinc-700/70 hover:bg-zinc-900/50 sm:flex-row sm:items-start"
      style={{ boxShadow: "0 1px 0 rgba(255,255,255,0.03) inset" }}
    >
      <div className="flex h-12 w-12 shrink-0 items-center justify-center rounded-lg border border-zinc-800/60 bg-zinc-900/60 backdrop-blur-sm">
        <Building2 className="h-5 w-5 text-zinc-400" />
      </div>

      <div className="min-w-0 flex-1">
        <div className="mb-1 flex flex-wrap items-center gap-2">
          <h2 className="text-[16px] font-semibold text-white">{job.title}</h2>
        ...
        <div className="mb-3 flex flex-wrap items-center gap-4 font-mono text-[10px] uppercase tracking-wider text-zinc-600">
          {job.location && (
  ...
  return (
    <div className="flex items-center justify-center gap-1">
      <button
        ...
        disabled={page === 1}
        className="flex h-8 w-8 items-center justify-center rounded-lg border border-zinc-800/60 bg-zinc-900/60 text-zinc-400 transition-all hover:border-zinc-600/60 hover:text-white disabled:cursor-not-allowed disabled:opacity-30"
      >
      ...
      {pages.map((p, i) =>
        p === "…" ? (
          ...
          <button
            ...
            onClick={() => onChange(p)}
            className={[
              "flex h-8 w-8 items-center justify-center rounded-lg font-mono text-[11px] transition-all",
              p === page
                ? "bg-white text-black"
...
export function JobsClient({ allJobs, topMatches, hasProfile, watchlistedIds }: Props) {
  ...
  return (
    <div className="mx-auto max-w-4xl space-y-6 px-4 py-8">
      ...
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center">
        <div className="relative flex-1">
          ...
          <input
            ...
            className="w-full rounded-lg border border-zinc-800/60 bg-zinc-900/60 py-2.5 pl-9 pr-4 text-sm text-white placeholder:text-zinc-600 backdrop-blur-sm transition-all focus:border-zinc-600/60 focus:outline-none"
          />
        </div>

        <div className="flex shrink-0 items-center gap-1 rounded-lg border border-zinc-800/60 bg-zinc-900/60 p-1 backdrop-blur-sm">
          <button
            onClick={() => handleToggle(false)}
            className={[
              "flex items-center gap-1.5 rounded-md px-3 py-1.5 font-mono text-[10px] uppercase tracking-wider transition-all",
              !showAll ? "bg-white text-black" : "text-zinc-500 hover:text-zinc-300",
            ].join(" ")}
          >
      ...
      {/* No profile notice */}
      {!hasProfile && !showAll && (
        <div className="flex items-center gap-3 rounded-lg border border-amber-500/20 bg-amber-500/[0.06] p-4 text-sm text-amber-300/80">
          <Triangle className="h-4 w-4 shrink-0 fill-current" />
      ...
      {filtered.length === 0 ? (
        <div className="rounded-lg border border-dashed border-zinc-800/60 py-20 text-center backdrop-blur-sm">
          ...
          <p className="mt-1 text-sm text-zinc-600">
            {query ? "Try a different keyword" : "Check back soon for new opportunities"}
          </p>
        </div>
      ) : (
