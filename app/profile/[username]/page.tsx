...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const [profile] = await db
    .select({ fullName: candidateProfiles.fullName, bio: candidateProfiles.bio })
  ...
  return {
    ...
    description: profile.bio ?? `View ${profile.fullName ?? username}'s profile on SkillSync.`,
    openGraph: {
      title: `${profile.fullName ?? username} — SkillSync`,
...
export default async function PublicProfilePage({ params }: Props) {
  ...
  const resumeInsights = candidate.resumeInsights as { strengths?: string[]; improvements?: string[] } | null;
  const skills: string[] = (resumeData?.skills as string[] | undefined) ?? [];
  const strengths = Array.isArray(resumeInsights?.strengths) ? resumeInsights!.strengths : [];
  const improvements = Array.isArray(resumeInsights?.improvements) ? resumeInsights!.improvements : [];
    ...
    <div className="min-h-screen bg-black">
      ...
      <main className="mx-auto max-w-4xl space-y-6 px-4 py-10">
        ...
        >
          ...
          <div className="relative flex flex-col justify-between gap-6 md:flex-row md:items-start">
            <div className="flex items-start gap-4">
              <UserAvatar imageUrl={candidate.imageUrl} name={candidate.fullName ?? candidate.email} className="h-16 w-16" textClassName="text-lg" />
              <div>
                <p className="font-mono text-[10px] uppercase tracking-[0.22em] text-zinc-500">candidate profile</p>
                <h1 className="mb-1 mt-1 text-2xl font-bold text-white">{candidate.fullName}</h1>
                <p className="mb-3 max-w-md text-sm leading-relaxed text-zinc-400">{candidate.bio || "No bio yet."}</p>
                <div className="flex flex-wrap gap-2">
                  ...
                  {candidate.githubUsername && (
                    <a
                      ...
                      className="inline-flex items-center gap-1 rounded-md border border-zinc-800 bg-black px-3 py-1.5 font-mono text-xs uppercase tracking-wider text-zinc-300 transition-colors hover:text-white"
                    >
        ...
        <div className="grid gap-4 md:grid-cols-2">
          {/* Skills */}
          {skills.length > 0 && (
            <section className="rounded-2xl border border-zinc-800/60 bg-zinc-950/60 p-6">
                ...
                return (
                  <div key={roadmap.id} className="space-y-2">
                    <div className="flex items-center justify-between">
                      ...
                      <span className={`font-mono text-[10px] tabular-nums uppercase tracking-wider ${pct === 100 ? "text-emerald-400" : "text-zinc-500"}`}>
                        {pct}%
                      </span>
                    </div>
                    <div className="h-1.5 overflow-hidden rounded-full bg-zinc-800">
                      <div
                        className={`h-full rounded-full transition-all ${pct === 100 ? "bg-emerald-400" : "bg-white"}`}
                        style={{ width: `${pct}%` }}
                      />
                    </div>
                  </div>
          ...
          <section className="rounded-2xl border border-zinc-800/60 bg-zinc-950/60 p-6">
            ...
            <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
              {[
                ...
                { label: "Score", value: `${githubData.developerScore}/100` },
              ].map(({ label, value }) => (
                <div key={label} className="rounded-xl border border-zinc-800/60 bg-zinc-900/40 p-3 text-center">
